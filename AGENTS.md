...
- **Prove.** Show it works by running it on the path a real user takes. A safety net (a test, a check, an alert) counts as evidence only after you've watched it fail on purpose once. Substantial work then gets an independent review: use the `independent-review` skill. The summons is structural, not optional: nothing reaches Proven without the review's receipt, and at most one substantial piece may sit unproven when the next one starts — review debt is bounded, so momentum can never outrun the auditor.
- **Leave the state clear.** Update `state.md` by its rules; if the work taught something a future agent shouldn't relearn the hard way, write it into the work file. Then stop or take the next piece. While the owner is in the room, the beat tightens: `state.md` re-lands at every event that changes the map — a piece starting or ending, a ruling landing or reopening work — and every reply names where we are on the map, because the owner should never have to ask. A ruling that invalidates running or finished work gets a trace: its decision entry names the pieces it reopens, and each reopened work file points back at it.

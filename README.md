...
Being built toward v1, and self-hosted: this repository runs under its own [AGENTS.md](AGENTS.md), so every development session here is a live test of the method. The development suite is green with checks proven able to fail ([devsuite/](devsuite/)); a fresh cross-vendor agent passed governance unprompted. v1 is accepted on demonstrated confidence — owner decision — with its target the greenfield reboot of a real product; the designed head-to-head contest is shelved but kept ([docs/benchmark/fixtures.md](docs/benchmark/fixtures.md)). Current truth and next steps: [state.md](state.md). Why the clean sheet: [docs/history/](docs/history/). How these documents were attacked: [docs/reviews/](docs/reviews/).

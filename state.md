...
The live piece per [map.md](map.md): **v4 experience→judge — judged BROKEN on first pass, fixes landed, re-witness/re-judge running**. The first run of the new law caught its own leaks: the witness's walk still ordered judgment, the judge's boundary had an execution-shaped sentence, the owner-facing pages failed the one-read bar, receipt skeletons dropped fields, the README narrowed the method, the install count had an unstated boundary — all fixed; the ratification question above is yours. Then: tag v4.0.0, upgrade Odd. The judge also ordered three experiences that land later: a real UI piece in all four personas and a full fresh-install lifecycle run (both = Odd's build), and the promise-7 CI controls (the queued CI piece). Queued behind triggers: the v11 converter, CI limit enforcement, the promise-conservation check.

## Evidence

This repo's history, 2026-08-13 → 19: the v4 design riff and the owner's three verbatim calls in [decisions.md](decisions.md); the shape committed before the code (work/v4-experience-judge.md, its own commit); the full first witness record ([docs/reviews/v4-witness-record.md](docs/reviews/v4-witness-record.md)) and judge rulings ([docs/reviews/v4-judge-rulings.md](docs/reviews/v4-judge-rulings.md)), both receipted at dispatch; the dev suite control re-run 4-of-4 red after the v4 regex changes; install measured 19 files on disk / 55 KB, always-read 15.7 KB.

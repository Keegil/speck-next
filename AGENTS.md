...
2. **Set up the piece.** Before any model-bearing work on the piece, commit its work file (start from `templates/piece.md`) with the outcome, proof plan, a token estimate, hard authorization in model turns or contexts, elapsed time, retries and fallbacks, owner interruptions, and time and files read before the first run, plus the four role-call decisions. Before product code, add the contributions from every called role. Product and Engineering always use distinct carriers. Product records one synthesis with dissent preserved, then hands implementation to Engineering.
3. **Build while running it.** Check the declared authorization before every model-bearing turn. Exhausting any part forbids another one. Measure gross, cached, and fresh tokens against the estimate as Business cost evidence; crossing it is a cost finding, not a product verdict. If planning has gone on for a long time and nothing has run, stop planning and get the smallest honest part running. Return each active role at its named earliest informative run and record what the evidence changed.

...
The v4 piece is **Judged and landed** (tag v4.0.0; Odd upgraded). The milestone's proving continues on Odd's build — five phases, verdict-free records, separate judges, measured by the playbook's git checks and the owner's felt verdict — which also carries two of the re-judge's ordered experiences (a real four-persona UI piece; a full fresh-install lifecycle with one deliberately insufficient judgment routed backward). The third ordered experience is the queued CI piece (promise-7 controls watched red). Queued behind triggers: the v11 converter, CI limit enforcement, the promise-conservation check.

...
The pre-fix positive/negative pair is mandatory: balanced `` `<!--` `` before a current assessment changes from false refusal to the exact pending or completed route, while an unmatched opener before a real comment remains an atomic ambiguity refusal. The widened table covers one-, two-, and three-backtick spans; shorter internal runs; multiline spans; hidden exact generated and canonical lines; odd/even backslash parity; backslash before a closer; wrong-length and unmatched runs; every named block boundary; comment-first input; quote/fence isolation; a real comment after a balanced span; ordinary and flagged recovery preserving the literal example. Each is an integration subject. All 26/8/95 prior subjects remain individually green.

...
The live piece per [map.md](map.md): **v3 discipline is in prove** — its proving run is Pilot's next sessions (re-map as ratified rounds, then build under the phase contracts), measured by the playbook's git checks and by the only judge that matters. Queued behind triggers: the v11 converter, CI limit enforcement, the promise-conservation check.

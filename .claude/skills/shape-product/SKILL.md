...
Before Product integrates `product.md`, Product, Business, Experience, and Engineering each contribute from a distinct context. This applies when the product is first shaped and whenever Shape is reopened. Record each carrier, direct evidence, conclusion, assumptions, proposed change, and active decision in `work/shaping.md`. Business and Experience each author the observable conditions that will call them on later work, the direct evidence trusted to keep them out, when that evidence expires, and examples of material change. A role claiming material impact names the consequence and the earliest run that could test its conclusion; Product cannot mark that concern inactive. Product preserves meaningful dissent and gives the owner one integrated explanation. The roles do not replace the fresh tester and judge below, and none of their contributors is eligible for either job.
...
12. **Name this product's responsibilities.** In `product.md`, say what Product, Business, Experience, and Engineering protect for this product. Business and Experience also state their observable call conditions, trusted current evidence, evidence expiry, and material-change examples; uncertainty later calls the relevant role. Add a specialist only for a product-specific need, with one responsibility and the observable trigger that ends the assignment.

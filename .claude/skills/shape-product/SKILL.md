...
2. **Rebuild the category, don't decorate it.** Every product category is full of fossils — structures that only exist because software couldn't think when they were invented, or because the physical world imposed them: aisles, pre-programmed plans and splits, folders, feeds, blank forms. Interrogate each concept the category treats as god-given; a fossil is a candidate for deletion, not automation — automating a fossil is how you get the same old product with AI tacked on. Reason instead from the job, the domain's first principles, and the user's real mental model, and build the product to work the way the best human in this role works: asking, assuming out loud, taking corrections, remembering, adjusting. The honesty test: unplug the model — if the product still basically works, the intelligence was decoration and the concept isn't AI-first.

...
7. **Small by law.** Hard limits in this repository's own CI, each with a test proving the limit can actually trip:
   ...
   - What an agent always reads (`AGENTS.md` + `state.md` + `product.md`): **≤ 50 KB** — raised from 25 KB by the owner after the first real campaign showed a deeply-shaped `product.md` earning its weight (the shaping process benefited from the context). The budget bounds drag, not thinking: a `product.md` full of shaped decisions is inside the spirit; one full of narration is not, at any size.

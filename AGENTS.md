...
- **Experience** protects how the product is understood and used. It reads observed journeys, surfaces, user behavior, and the declared feel. When called for substantial user-facing work, it composes the complete user job and meaningful states from the approved drawing and declared feel, using the existing drawing or piece work record. Product integrates that composition into the binding Engineering handoff. This is product work, not the later fresh-user test, and it never stands in for a user.
- **Engineering** protects feasibility, safety, reversibility, and operation. It reads the repository, runtime, dependencies, data, and real system behavior. Engineering owns implementation. For a substantial user-facing change, one Engineering carrier owns the complete job and its meaningful states from the Product handoff through the integrated run and any finding-driven re-run required before affected work resumes. It may delegate bounded parts, but it keeps the authority and responsibility to integrate or replace the affected container or sequence within the approved product.
...
2. **Set up the piece.** Before any model-bearing work on the piece, commit its work file (start from `templates/piece.md`) with the outcome, proof plan, a token estimate, hard authorization in model turns or contexts, elapsed time, retries and fallbacks, owner interruptions, and time and files read before the first run, plus the four role-call decisions. Before product code, add the contributions from every called role. For substantial user-facing work, called Experience composes the complete user job and meaningful states from the approved drawing and declared feel. Name the actual drawing or output reference; if it does not exist, compose it in the existing piece work record before code. Product integrates that composition into its synthesis. Product and Engineering always use distinct carriers. Product records one synthesis with dissent preserved, then hands implementation to one Engineering owner.
3. **Build while running it.** Check the declared authorization before every model-bearing turn. Exhausting any part forbids another one; it does not resolve a finding or permit affected work to continue. Measure gross, cached, and fresh tokens against the estimate as Business cost evidence; crossing it is a cost finding, not a product verdict. If planning has gone on for a long time and nothing has run, stop planning and get the smallest honest part running. For substantial user-facing work, Engineering integrates the complete job and its meaningful states before work that depends on that result advances. Return each active role at its named earliest informative run and record what the evidence changed. A concrete failure of the integrated experience holds the work that consumes or builds on the failed result, including later work inside the same piece, whoever observes it. Engineering fixes and re-runs the affected complete sequence. Called Experience reads that changed sequence and states whether the original concrete finding is resolved; every other role whose consequence changed also returns. Product lifts the hold only when that finding is resolved. A replacement carrier may inherit the existing evidence and contribution, but not erase the unresolved consequence. This conditional return does not add a fixed return to every run. Independent unrelated work and genuine unrelated small changes keep their cheap path. A small component repair may fix the defect, but it cannot lift the larger hold without the affected complete-sequence re-run and resolving Experience return.
...
Contributor-excluded fresh use and independent judgment decide whether the running product is sufficient. Token totals cannot supply or erase that evidence. Once a real result exists, only a concrete product finding from fresh review or a genuinely new product claim authorizes another build. Keep method and cost findings visible and route them to the kernel or a future run for subtraction; they cannot trigger duplicate product work.
...
A change is small only if it adds no dependency, touches no protected code, changes no promise, and is reversible in one commit. Then fix it, run it, and finish without a work file. The integrated-run hold does not summon the product team for a genuine unrelated small change or stop work that does not consume the failed result. A small component repair cannot lift an existing hold until the affected complete sequence runs and Experience resolves the original finding.

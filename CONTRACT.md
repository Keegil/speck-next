...
Status: **v0.3 — revised against adversarial review round 1 and the round-2 closure check** (three decorrelated reviewers + one closure verifier; see [docs/reviews/](docs/reviews/)). The closure check confirmed 14 of 16 round-1 P0s closed; this version closes the remaining one and the three it introduced. Next: Lock 2.
...
3. "The 'owner involvement' should not come at the expense of less LARP. The agent actually using and testing the product and having a feedback loop around this is still absolutely critical."

## Promises

1. **Iterative to the bone.** Work ships as small increments that each end in something runnable, and the owner operates the real surface — a demo is handed over, not merely produced. The agent's own use of the product is the primary feedback loop: throughout every increment the agent runs the thing, uses it as the user would, and iterates on what it finds — a demo reaches the owner only after the agent has used it itself, and owner touchpoints add steering on top of that loop, never replace it (owner constraint 3). Measured, not asserted: minutes, tokens, and files written **before the product first executes on the real path** are capped per unit of work; an owner steer must be able to materially change the next increment. Where the real action is irreversible (payments, live recipients, production data), the round-trip runs against a named replica and the fidelity gap is disclosed in the evidence. *Acceptance: pre-first-run caps, evidence of agent-run product sessions preceding every demo, and logged owner touchpoints scored on T02–T04 and T09.*
...
1. **Any non-compensatory gate fails at a predeclared slice** — the falsifier is exactly the gate list, nothing narrower: per-class truth-detection vs the frozen v11.2.0 baseline, on frozen **and** held-out plantings · adoption fidelity across T11–T14, T18, and the synthetic-era set (lossy residue, manual repair, or legacy knowledge needed = fail) · the product-quality floor on all four axes · no-overclaim · every Promise 7 budget (no quality-loss escape clause; an overrun falsifies).
2. **Adoption in anger**: if no consumer repository is doing real product work on speck-next by **2026-10-01**, the thesis has failed regardless of benchmark score. Owner decides continuation explicitly; silence means failure.

...
The live piece per [map.md](map.md): **v4 experience→judge — built, being judged**: the kernel change gets witnessed and judged under its own new law (bootstrap: the first real run of the two skills), findings closed, then the tag and Odd's upgrade. After that the milestone's proving is Odd's build — five phases, verdict-free records, separate judges, measured by the playbook's git checks and the owner's felt verdict. Queued behind triggers: the v11 converter, CI limit enforcement, the promise-conservation check.
